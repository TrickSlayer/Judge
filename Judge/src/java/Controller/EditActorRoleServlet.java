 ...
 * @author Acer
 */
@WebServlet(name = "EditActorRole", urlPatterns = {"/edit-actor-role"})
public class EditActorRoleServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditActorRole</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditActorRole at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<ActorRole> listActorRole = m.getActorByMovieID(id);
        request.setAttribute("listActorRole", listActorRole);
        request.getRequestDispatcher("EditActorRole.jsp").forward(request, response);
    }
        ...
        if (action.equals("Search")) {
            ...
            if (action.equals("addOldActor")) {
                ...
                if (action.equals("addActor")) {
                    ...
                    Date DOB = null;
                    try {
                        DOB = (Date) new SimpleDateFormat("dd/MM/yyyy").parse(request.getParameter("DOB"));
