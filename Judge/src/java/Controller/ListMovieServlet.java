...
public class ListMovieServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("list", m.getListMoviePage(page,order));
        request.setAttribute("page", m.size() / 10 + (m.size() % 10 > 0 ? 1 : 0));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (part[0].equals("Delete")) {
            ...
            try {
                ...
                mess = m.deleteMovie(id, realPath);
                response.sendRedirect("list?page=1&order=desc");
            } catch (Exception e) {
            }
            out.print(mess);
        } else {
            response.sendRedirect("edit-movie?id=" + id);
        }
    }
